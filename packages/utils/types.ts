 ...
 *
 * @example
 * BpPropMergeType<StringConstructor, '1', 1> =>  1 | "1" // ignores StringConstructor
 * BpPropMergeType<StringConstructor, never, number> =>  string | number
 */
export type BpPropMergeType<Type, Value, Validator> =
 ...
 * 处理输入参数的默认值（约束）
 */
export type BpPropInputDefault<
 ...
 *
 * @example
 * BpPropInput<StringConstructor, 'a', never, never, true>
 * ⬇️
 * {
 ...
 */
export type BpPropInput<
  Type,
  Value,
  Validator,
  Default extends BpPropMergeType<Type, Value, Validator>,
  Required extends boolean
> = {
  type?: Type
  required?: Required
  ...
  default?: BpPropInputDefault<Required, Default>
}
 ...
 */
export type BpProp<Type, Default, Required> = {
...
/**
 * Determine if it is `BpProp`
 */
export type IfBpProp<T, Y, N> = T extends { [epPropKey]: true } ? Y : N
 ...
 * 将输入转换为输出
 */
export type BpPropConvert<Input> = Input extends BpPropInput<
  infer Type,
  infer Value,
  infer Validator,
  any,
  infer Required
>
  ? BpPropFinalized<Type, Value, Validator, Input['default'], Required>
  : never
 ...
 * 最终转换 BpProp
 */
export type BpPropFinalized<Type, Value, Validator, Default, Required> = BpProp<
  BpPropMergeType<Type, Value, Validator>,
